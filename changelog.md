...
- moodle-reviewer finding (2026-07-06): `notifier::notify_submission_created()`/
  ...
  `submissiontitle`/`coursename` placeholders on the same lines were correctly
  passed through `format_string()`. Since the default template body format is
  FORMAT_HTML, a display name containing markup would have rendered raw in the
  resulting email. Fixed by wrapping both in `format_string()`, matching the
  existing placeholders' escaping. The identical pattern (copied from this
  plugin into the sibling `mod_confprogram`/`mod_confscheduler` notifiers) was
  fixed the same way in both of those plugins in the same pass. 45/45 PHPUnit
  passing, phpcs clean.
